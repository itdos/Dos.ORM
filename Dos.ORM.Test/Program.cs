...
namespace Dos.ORM.Test
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("------------------------------------------------------------------");
            //return;
            var db2016102911111 = new DbSession(DatabaseType.SqlServer9, @"server=47.93.101.25;database=ITdos.Project;uid=nb;pwd=nb.ITdos.com;");
            var tableName = Guid.NewGuid().ToString().Replace("-", "");
            var c1 = db2016102911111.FromSql("create table #" + tableName + " (gid varchar(100),cardno varchar(200));insert into #" + tableName + " values(1,2);insert into #" + tableName +
                              " values(1,2);insert into #" + tableName + " values(1,2);").ExecuteNonQuery();
            //var c2 = db2016102911111.FromSql("").ExecuteNonQuery();
            var c3 = db2016102911111.FromSql("drop table #" + tableName + "").ExecuteNonQuery();
